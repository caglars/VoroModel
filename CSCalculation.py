...
class CSCalculator():
    ...
    def getPermeabilityNew(self, aDataFrame, cellId, neighborId):
    ...
    def getFormationVolumeFactorNew(self, aDataFrame, cellId, neighborId):
        # explicit treatment of the parameters, pressures are at time step n
        cellVolume = aDataFrame['volumes'][cellId]
        neighborVolume = aDataFrame['volumes'][neighborId]
        #todo Factor is calculated using bulk volumes, it would be better to use pore volumes
    ...
    def getViscosityNew(self, aDataFrame, cellId, neighborId):
        # explicit treatment of the parameters, pressures are at time step n
        cellVolume = aDataFrame['volumes'][cellId]
        neighborVolume = aDataFrame['volumes'][neighborId]
        #todo Factor is calculated using bulk volumes, it would be better to use pore volumes
    ...
    def simRunSlightlyCompressibleNew(self, aContainer):
        #self.particles = numberOfParticles
                ...
                for cellIndex in range(0, len(myDataFrame)):
                    ...
                    for neighborIndex in range(0, numberOfNeighbors):
                        ...
                        if neighbor >= 0:
                            ...
                            self.permeability = self.getPermeabilityNew(myDataFrame, cellIndex, neighbor)
                            coefficient[cellIndex][neighbor] = (self.betaConstant
                                                                * myDataFrame['faceAreas'][cellIndex][neighborIndex]
                                                                * self.permeability) / (self.getViscosityNew(myDataFrame, cellIndex, neighbor)
                                                                  * self.getFormationVolumeFactorNew(myDataFrame, cellIndex, neighbor)
                                                                  * length)
